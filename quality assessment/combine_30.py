"""
 ...
 @FileName: combine_30.py
 @Function: 
 @DateTime: 2022/11/13 9:02
 @SoftWare: PyCharm
"""
...
if __name__=="__main__":
    ...
    saveroot = r"E:\研一\信号质量评估\result4.1\classify_result4.1未调参\30s/"
    # outputpath = r"E:\研一\信号质量评估\Result_new\assessment\30s/"
    ...
    for filename in filenames:
        ...
        # if file_num == '1006' or file_num == '549' or file_num == '971' or file_num == '1296' or file_num == '1010' or file_num == '582' or \
        #         file_num == '584' \
        #         or file_num == '586' or file_num == '735':
        #     print("%s 的文件无法进行5分类" % file_num)
        # else:
        feature_file = np.loadtxt(os.path.join(filepath, filename + '.txt'), encoding='utf-8')
        ...
        for k, (train, test) in enumerate(kfold):
            ...
            # y_pred1 = np.round(RF.predict(X_test))
        ...
        score_result = [(file_num, np.mean(scores_rf), np.mean(scores_dt), np.mean(scores_knn), np.mean(scores_xg),
                         np.mean(scores_lgb), np.mean(scores_cb), np.mean(scores_ada))]
