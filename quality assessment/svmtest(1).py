...
def load_data():
    # 导入数据
    data = np.loadtxt('D:/assess_result/new/module_all.txt', dtype=float, delimiter=None, converters={7: iris_type})
    return data
...
# 训练四个特征：
data = load_data()
x, y = np.split(data, (7,), axis=1)  # x为前四列，y为第五列，x为训练数据，y为数据标签
...
y_predicted = train(clf, x_train, y_train)  # 启动分类器进行模型训练
print_accuracy(clf, x_train, y_train, x_test, y_test)
y_pred = clf.predict(x_test)
y_pro = clf.predict_proba(x_test)

i = 'KNN'
print(f'{i}  准确率为:',accuracy_score(y_test,y_pred))
print(f'{i}  平均准确率为:',precision_score(y_test,y_pred, average='weighted'))
print(f'{i}  召回率为:',recall_score(y_test,y_pred, average='weighted'))
print(f'{i}  F1指数为:', f1_score(y_test,y_pred, average='weighted'))
