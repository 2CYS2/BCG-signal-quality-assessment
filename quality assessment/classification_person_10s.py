...
if __name__ == '__main__':
    ...
    saveroot = r"E:\研一\信号质量评估\result12.5\normalization\留一法(未级联)-n_estimators=10剔除5个特征/"
    ...
    for k in range(0, 40):
        ...
        # feature_test = feature_test[feature_test[:, 19].argsort()]  # 将所有片段按质量标签降序排列，【若是未级联，则不用重排】
        X_train = feature_train[:, (0, 1, 2, 6, 7, 8, 9, 10, 11, 12, 14, 15, 16, 19, 20)]  # 1:21
        y_train = feature_train[:, 21]
        X_test = feature_test[:, (0, 1, 2, 6, 7, 8, 9, 10, 11, 12, 14, 15, 16, 19, 20)]  # 1:21
        ...
        error_index = np.where(y_pred != y_test)[0] + 1
        ...
        # 保存分类混淆矩阵
        plot_matrix(y_test, y_pred, [1, 2, 3], title='Confusion_matrix', axis_labels=['a', 'b', 'c'])
        ...
        if len(y_test):
            ...
            # 特征删除前：0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 13, 15, 16
            # 特征删除后：0, 1, 5, 7, 8, 9, 10, 11, 13, 15
            RF1.fit(X_train[:, (0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 13, 15, 16)], y_train)
            y_pred_c2 = np.round(RF1.predict(X_test[:, (0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 13, 15, 16)]))
        ...
        if len(y_test):
            ...
            # # 特征删除前：0, 1, 2, 3, 6, 9, 10, 12, 15, 17, 18, 19
            # RF2.fit(X_train[:, (0, 1, 6, 9, 10, 12, 15, 18, 19)], y_train)
            # y_pred_b1 = np.round(RF2.predict(X_test[:, (0, 1, 6, 9, 10, 12, 15, 18, 19)]))
            # 使用LightGBM
            model = XGBClassifier(n_estimators=500, random_state=42)
            # 特征删除前：0, 1, 2, 3, 6, 9, 10, 12, 15, 17, 18, 19
            # 特征删除后：0, 1, 6, 9, 10, 12, 15, 18, 19
            model.fit(X_train[:, (0, 1, 2, 3, 6, 9, 10, 12, 15, 17, 18, 19)], y_train)
            y_pred_b1 = np.round(model.predict(X_test[:, (0, 1, 2, 3, 6, 9, 10, 12, 15, 17, 18, 19)]))
